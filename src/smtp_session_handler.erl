...
init(Hostname, SessionCount, Address, Options) ->
	...
	case SessionCount > 20 of
		false ->
			Banner = [Hostname, " ESMTP Erlmail smtp server"],
...
handle_AUTH(Type, Username, Password, State) when Type =:= login; Type =:= plain ->
	case dummy_auth:auth(binary_to_list(Username),binary_to_list(Password)) of
		ok -> {ok,State};
		_ -> error
	end;
